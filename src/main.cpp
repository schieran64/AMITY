...
unsigned int nStakeMinAge = 10 * 24 * 60 * 60; // 10 days
unsigned int nStakeMaxAge = 60 * 24 * 60 * 60; // 60 days
...
const string strMessageMagic = "AMITY Signed Message:\n";
...
int64_t GetProofOfWorkReward(int64_t nFees)
{
    ...
    nSubsidy >>= ((pindexBest->nHeight+1) / 2628000);
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    nRewardCoinYear >>= ((pindexBest->nHeight+1) / 2628000);
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "AMITY", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Tue, 03 May 2016 17:23:42 GMT";
        CTransaction txNew;
        txNew.nTime = 1462296222;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1462296226;
        block.nBits    = bnProofOfWorkLimit.GetCompact();
        block.nNonce   = 1435159;
		if(fTestNet)
        {
            block.nNonce   = 1435159;
        }
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("0x169db5f9b0763aa3bc8d99f2b22004c78264dddbde2d2a227980e595777bc92c"));
        block.print();
